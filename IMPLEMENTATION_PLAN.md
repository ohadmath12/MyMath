# MyMath — אפיון ותוכנית פעולה לטופס הרשמה
...
לבנות עבור MyMath עמוד הרשמה עצמאי, מקצועי ורספונסיבי, בהשראת המבנה הפונקציונלי של עמוד ההרשמה שנבדק אך בעיצוב מקורי לחלוטין.
...
13. פרטי ההתקשרות—כתובת, טלפון, דוא״ל ואינסטגרם—יישארו כערכים ריקים וקלים לעריכה.
...
- לפני פרסום אמיתי יש לוודא של־MyMath יש זכות להשתמש בכל נוסח סופי, בשם המותג ובכל נכס שיוכנס בעתיד.
...
```mermaid
flowchart TD
    A["הורה או תלמיד פותח את MyMath Web App"] --> B["Index.html"]
...
- לוגו טקסטואלי/גאומטרי מקורי: `MyMath`.
...
יוכן אזור עם אייקונים וערכי placeholder ריקים:
...
כל הערכים ירוכזו באובייקט הגדרות אחד בראש קוד הלקוח כדי שניתן יהיה להחליפם ללא חיפוש בכל הקובץ.
...
| תלמיד | כתובת דוא״ל | `student_email` | דוא״ל | — | כן |
...
2. יצירת Google Sheet בשם MyMath Registrations.
3. יצירת לשונית `Registrations` והוספת הכותרות המוגדרות.
4. הגדרת עמודות תעודת זהות וטלפון כטקסט.
5. יצירת תיקיית Drive פרטית בשם MyMath Signatures.
